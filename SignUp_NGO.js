...
ws.on('message', function incoming(data) {
	...
	if (data.command == 'register:ack'){
		...
		NGOInstance.post('/sign_up', {
			fundraiser_name: "NGO3", //Input from FrontEnd
			fundraiser_target: 25000, //FrontEnd Input
			index: 3,	//Autoincrement index pick from backend
			owner: 2 //0 if owner, otherwise index of Owner
		})
	...
	if (data.type == 'event' && data.event_name == 'SignUp'){
		...
		var myobj = { index : data.event_data["index"],fundraiser_name : data.event_data["name"],target: data.event_data["target"],isOwner: owner_check,Owner: data.event_data["Owner"]};  
